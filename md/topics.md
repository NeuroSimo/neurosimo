...
- `/recording/streaming/start` - Client-side reference found; likely should be `/eeg_replay/streaming/start`.
- `/essor/heartbeat` - UI reference appears to be a typo (likely `/preprocessor/heartbeat`).
- `/eeg_bridge/dropped_samples` - UI subscription found; no backend publisher located.
- `/eeg/is_streaming` - Appears in mock/test helper context.
